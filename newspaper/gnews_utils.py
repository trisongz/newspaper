...
class OrJson:
    ...
    def loads(cls, obj, *args, **kwargs):
        return orjson.loads(obj, *args, **kwargs)
...
class GNewsArticle(BaseModel):
    ...
    def dumps(self, props: List[str] = None):
        return self.json(include=props)
...
class GNewsResult(BaseModel):
    ...
    description: Optional[str]
    published_date: Optional[str]
    publisher: Optional[Dict[str, str]]
    built: bool = False
    ...
    def publisher_name(self):
        return self.publisher.get('title', '')
    
    @property
    def publisher_url(self):
        return self.publisher.get('href', '')

    def get_article(self) -> Article:
        if not self.article:
            self.article = Article(url=self.url, title=self.title, source_url=self.publisher_url)
        return self.article
    ...
    @property
    def built_article(self):
        r = {
            'url': self.url,
            'title': self.title,
            'text': cleanup_text(self.article.text),
            'summary': cleanup_text(self.article.summary),
            'description': self.description,
            'publisher': self.publisher_name,
            'published_date': self.published_date or self.article.published_date,
            'images': list(self.article.images),
            'keywords': list(self.article.keywords),
            'num_words': self.article.num_words,
            'num_sentences': self.article.num_sentences,
        }
        return GNewsArticle(**r)
    ...
    async def async_dumps(self, props: List[str] = None):
        """ method for calling async article.async_build() and dumping resulting dict"""
        rez = await self.async_build()
        return rez.dumps(props=props)
